...
## <a name="new-for-ssis-in-azure-data-factory"></a>Новые возможности служб SSIS в Azure Data Factory

В общедоступной предварительной Azure Data Factory version 2, выпущенной в сентябре 2017 г., теперь можно выполнять следующие действия:
-   развертывать пакеты в базе данных каталога служб SSIS (SSISDB) в базе данных SQL Azure;
-   запускать пакеты, развернутые в Azure, в Azure-SSIS Integration Runtime, компоненте фабрики данных Azure Data Factory version 2.
...
####  <a name="IncrementalDeployment"></a> Инкрементальное развертывание пакетов  
Функция инкрементального развертывания пакетов позволяет добавлять один или несколько пакетов в существующий или новый проект без развертывания всего проекта. Для этого можно использовать следующие средства:  
...
####  <a name="ErrorColumn"></a> Имена столбцов для ошибок в потоке данных  
 При выводе строк с ошибками, в результате вывода содержатся числовые идентификаторы столбцов с ошибками, а не их имена. Теперь существует несколько способов для поиска или отображения имени столбца, где произошла ошибка.  
...
####  <a name="SAPBW"></a> Выпуск коннектора для SAP BW для SQL Server 2016  
 Microsoft® Connector  для SAP BW для Microsoft SQL Server® 2016 выпущен в составе пакета дополнительных компонентов SQL Server 2016. Чтобы скачать компоненты набора дополнительных функций, см. страницу [Microsoft® SQL Server® 2016 Feature Pack](https://go.microsoft.com/fwlink/?LinkID=746297)(Пакет дополнительных компонентов Microsoft® SQL Server® 2016).
 
#### <a name="oracleteradata"></a> Выпуск коннекторов версии 4.0 для Oracle и Teradata
Были выпущены Microsoft Connectors версии&4;.0 для Oracle и Teradata. Чтобы скачать коннекторы, см. раздел [соединители Microsoft версии 4.0 для Oracle и Teradata](https://www.microsoft.com/download/details.aspx?id=52950).

### <a name="pdwau5"></a> Выпуск коннекторов для системы платформы аналитики (PDW) с обновлением 5
